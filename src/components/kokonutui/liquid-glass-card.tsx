...
export type LiquidButtonProps = Omit<ButtonProps, "children"> & {
  ...
  return (
    <Button className={cn(liquidButtonVariants({ liquidVariant }), className)} {...props}>
      <>
        ...
        <div className="pointer-events-none absolute inset-0 -z-10 rounded-full bg-zinc-100/5 backdrop-blur-md" />
...
const liquidGlassCardVariants = cva(
  "group relative overflow-hidden bg-background/4 backdrop-blur-[2px] transition-all duration-300",
  {
    variants: {
      glassSize: {
        sm: "p-4",
...
function LiquidGlassCard({ className, glassSize, children, ...props }: LiquidGlassCardProps) {
  return (
    <Card className={cn(liquidGlassCardVariants({ glassSize }), className)} {...props}>
      <div className="pointer-events-none absolute inset-0 -z-10 rounded-lg bg-zinc-100/5 backdrop-blur-md" />
...
export function NotificationCenter() {
  ...
  return (
    <div className="w-full max-w-sm">
      <LiquidGlassCard className="gap-3.5 rounded-3xl border border-zinc-200/50 p-4 shadow-xl dark:border-zinc-700/60 dark:from-zinc-900 dark:to-black">
        <div className="flex items-center gap-3">
          <div className="relative mr-2 mb-4 h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-linear-to-br from-pink-400 via-pink-300 to-rose-200 shadow-lg ring-1 ring-black/5 dark:shadow-xl"></div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="mt-1 flex items-center justify-between">
            <div className="flex items-center justify-center gap-2">
              <LiquidButton aria-label="Previous track" size="icon" variant="ghost">
                <ArrowLeft className="size-4" />
              </LiquidButton>
              <LiquidButton
                aria-label={isPlaying ? "Pause" : "Play"}
                className="h-11 w-11 rounded-full bg-transparent text-zinc-300 transition-colors hover:bg-zinc-200/5 hover:text-zinc-200"
              ...
              <LiquidButton
                ...
                className="h-10 w-10 rounded-full bg-transparent text-zinc-300 transition-colors hover:bg-zinc-200/70 dark:text-zinc-300 dark:hover:bg-zinc-800/70"
                size="icon"
                variant="ghost"
              >
